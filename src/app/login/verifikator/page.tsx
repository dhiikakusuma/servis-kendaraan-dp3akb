...
export default function LoginVerifikator() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/auth/verifikator", {
        method: "POST",
      ...
      <main className="flex-1 flex items-center justify-center px-6 pb-10">
        <Card className="w-full max-w-md">
          <CardHeader className="items-center text-center pb-2">
            ...
            <CardTitle>Masuk sebagai Verifikator</CardTitle>
            <CardDescription>
              Memverifikasi pengajuan service sebelum diteruskan ke kasubag.
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              <Link
                href="/login/pemohon"
                className="inline-flex items-center justify-center rounded-xl border border-zinc-200 py-2.5 text-xs font-medium text-zinc-700 hover:bg-zinc-50 transition"
              >
                Pemohon
              </Link>
              <Link
